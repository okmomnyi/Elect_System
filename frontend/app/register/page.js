...
export const metadata = { title: 'Create Account — Academic Vote' };
    ...
    <div className="min-h-screen bg-surface flex flex-col md:flex-row">
      ...
      <div className="hidden md:flex flex-col justify-between bg-primary text-on-primary w-[44%] p-14 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-6">
          ...
          <p className="text-on-primary/65 text-base leading-relaxed max-w-xs">
            Register with your university email and a secure password. A verification code completes your account setup.
          </p>
          <div className="flex flex-col gap-3">
            {[
              { icon: 'lock',          label: 'Password + OTP two-factor auth' },
              { icon: 'verified_user', label: 'University email verified'        },
              { icon: 'how_to_vote',   label: 'One secure vote per election'     },
            ].map(({ icon, label }) => (
      ...
      <div className="flex-1 flex items-center justify-center p-8 md:p-14">
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            <h2 className="font-headline font-extrabold text-3xl text-primary mb-2">Create your account</h2>
            <p className="text-on-surface-variant">
              Use your university email address. A 6-digit code will verify it.
            </p>
          </div>
          ...
          <p className="mt-8 text-center text-xs text-on-surface-variant">
            Having trouble?
            <Link href="/support" className="font-bold text-primary hover:underline ml-1">Contact IT Support</Link>
          </p>
        </div>
      </div>
    </div>
